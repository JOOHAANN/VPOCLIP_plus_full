"""Generation 04: low-capacity, transition-centric direct RL policies.

Generation 03 showed that a high-capacity interaction network can fit the
seen classes but is seed-sensitive on unseen classes.  These models restrict
the learnable signal to class-agnostic current-to-candidate geometry, with
small optional causal evidence interactions.  They always emit one legal
candidate by argmax; no gate or random fallback is present.
"""
...
OUT = ROOT / "work_dir/active_view_iterative_test/generation_04"
GENERATION = "generation_04"
TRAIN_POOL_MODE = "policy40"
EPOCHS = 40
BATCH_SIZE = 8192
UPDATES_PER_EPOCH = 48
VARIANTS = {
    "g04_linear_transition": "geometry Fourier transition + linear scorer",
    "g04_linear_causal": "geometry transition + causal evidence interaction",
    "g04_small_transition": "low-capacity nonlinear transition scorer",
    "g04_linear_pose_transition": "geometry transition + compact pose statistics",
}
...
def geometry_features(c):
    """Add periodic angle terms without relying on raw camera slot indices."""
    a = c[..., :2]
    d = c[..., 2:4]
...
class LowCapacityPolicy(nn.Module):
    def __init__(self, variant):
        super().__init__()
        self.variant = variant
        gdim = 11
        if variant.endswith("linear_transition"):
            self.head = nn.Linear(gdim, 1)
        elif variant.endswith("linear_causal"):
            ctx = 4 + 19
            self.head = nn.Linear(gdim + ctx + gdim * ctx, 1)
        elif variant.endswith("linear_pose_transition"):
            ctx = 4 + 3
            self.head = nn.Linear(gdim + ctx + gdim * ctx, 1)
        else:
            self.g = nn.Sequential(nn.Linear(gdim, 16), nn.LayerNorm(16), nn.GELU())
            self.head = nn.Sequential(nn.Linear(16, 16), nn.GELU(), nn.Linear(16, 1))

    def forward(self, state):
        g = geometry_features(state["candidate"])
        if self.variant.endswith("linear_transition"):
            return self.head(g).squeeze(-1)
        if self.variant.endswith("linear_causal"):
            ctx = torch.cat((state["quality"], state["evidence"]), -1)
            x = torch.cat((g, ctx[:, None].expand(-1, 4, -1),
                           (g.unsqueeze(-1) * ctx[:, None, None, :]).flatten(-2)), -1)
            return self.head(x).squeeze(-1)
        if self.variant.endswith("linear_pose_transition"):
            ctx = torch.cat((state["quality"], compact_pose(state["pose"])), -1)
            x = torch.cat((g, ctx[:, None].expand(-1, 4, -1),
                           (g.unsqueeze(-1) * ctx[:, None, None, :]).flatten(-2)), -1)
            return self.head(x).squeeze(-1)
        return self.head(self.g(g)).squeeze(-1)
...
def loss_for(variant, q, u, valid):
    ...
    if variant.endswith("small_transition"):
        return (listwise_loss(q, u, valid) + .20 * pairwise_loss(q, u, valid)).mean()
    # Regression gives the low-capacity transition models a calibrated target.
    ...
    return reg.mean() + .15 * pairwise_loss(q, u, valid).mean()
...
def train_variant(variant, train, val, present, vp, cfg, pool, hold, seen, proxy_eps, proxy_bank, device):
    ...
    eligible = four & torch.isin(train.labels, torch.tensor(pool, device=device))
    ...
    for seed in SEEDS:
        ...
        net = LowCapacityPolicy(variant).to(device)
        compiled = torch.compile(net, mode="reduce-overhead")
        opt = torch.optim.AdamW(net.parameters(), lr=2e-4, weight_decay=.05)
        best = -float("inf")
        for epoch in range(1, EPOCHS + 1):
            ...
            row = dict(version=variant, seed=seed, epoch=epoch,
                       ...
                       contexts=int(len(episodes)), four_view_only=True, selective=int(selective.sum()),
    ...
    result = dict(selection=selection, metrics=reports,
                  train_protocol="four-view only; low-capacity transition policy; no gate")
...
def main():
    ...
    cfg["iterative_test"] = dict(generation=GENERATION, variants=VARIANTS,
                                  epochs=EPOCHS, seeds=SEEDS, batch_size=BATCH_SIZE,
                                  updates_per_epoch=UPDATES_PER_EPOCH, four_view_only=True,
                                  objective="low-capacity transition ranking", movement_weight=0.,
                                  train_pool_mode=TRAIN_POOL_MODE, gating=False, policy_action_only=True, selection="validation_only",
    ...
    (OUT / "generation_04.py").write_bytes(Path(__file__).read_bytes())
    ...
    pool = (cfg["evaluation"]["seen_class_ids"] if TRAIN_POOL_MODE == "all50"
            else cfg["v6"]["policy_train_classes"])
