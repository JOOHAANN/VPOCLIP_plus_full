...
def loss_for(variant, q, u, valid):
    if variant.endswith("pairwise_aug") or variant.endswith("5way_pairwise"):
        return v9_loss("v9", q, u, valid)
    if variant.endswith("listwise_aug") or variant.endswith("5way_listwise") or variant.endswith("all50_listwise"):
        return (listwise_loss(q, u, valid) + .20 * pairwise_loss(q, u, valid)).mean()
    if variant.endswith("regression_aug") or variant.endswith("5way_regression") or variant.endswith("all50_regression"):
        return relative_loss(q, u, valid)
    if variant.endswith("oracle_ce"):
        safe = u.masked_fill(~valid, -1e4)
        best = safe.max(-1, keepdim=True).values
        target = ((safe >= best - .05) & valid).float()
        target = target / target.sum(-1, keepdim=True).clamp_min(1.)
        return (-(target * F.log_softmax(q.masked_fill(~valid, -1e4), -1)).sum(-1)).mean()
...
def train_variant(variant, train, val, present, vp, cfg, pool, hold, seen, proxy_eps, proxy_bank, device):
    ...
    for seed in SEEDS:
        ...
        for epoch in range(begin, EPOCHS + 1):
            ...
            banks = task_banks(train, episodes, pool, gen, TASK_FULL_FRACTION)
...
def main():
    ...
    cfg["iterative_test"] = dict(generation=GENERATION, variants=VARIANTS, epochs=EPOCHS,
                                 ...
                                 task_full_fraction=TASK_FULL_FRACTION, train_pool_mode=TRAIN_POOL_MODE,
    ...
    base_pool = cfg["v6"]["policy_train_classes"] if TRAIN_POOL_MODE == "policy40" else cfg["evaluation"]["seen_class_ids"]
    ...
    for variant in VARIANTS:
        pool = cfg["evaluation"]["seen_class_ids"] if variant.startswith("g02_all50") else base_pool
