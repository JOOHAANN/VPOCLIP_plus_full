...
class LowCapacityPolicy(nn.Module):
    def __init__(self, variant):
        ...
        gdim = 11
        if variant == "g04_linear_transition":
            self.head = nn.Linear(gdim, 1)
        elif variant == "g04_linear_causal":
            ctx = 4 + 19
            self.head = nn.Linear(gdim + ctx + gdim * ctx, 1)
        elif variant == "g04_linear_pose_transition":
            ctx = 4 + 3
            self.head = nn.Linear(gdim + ctx + gdim * ctx, 1)
        else:
    ...
    def forward(self, state):
        ...
        if self.variant == "g04_linear_transition":
            return self.head(g).squeeze(-1)
        if self.variant == "g04_linear_causal":
        ...
        if self.variant == "g04_linear_pose_transition":
...
def loss_for(variant, q, u, valid):
    ...
    if variant == "g04_small_transition":
...
def main():
    ...
    cfg["iterative_test"] = dict(generation="generation_04", variants=VARIANTS,
                                  ...
                                  gating=False, policy_action_only=True, selection="validation_only",
    ...
    pool, hold, seen = cfg["v6"]["policy_train_classes"], cfg["v6"]["policy_holdout_classes"], cfg["evaluation"]["seen_class_ids"]
    ...
    verify_v6()
    print("ITERATIVE_GENERATION_COMPLETE generation_04", flush=True)
