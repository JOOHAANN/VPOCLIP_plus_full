...
def evaluate(model, cache, present, classes, protocol, seed, device, episodes=None):
    if episodes is None:
        episodes = torch.arange(cache.num_episodes, device=device)
    n = len(episodes)
    rows = torch.arange(n, device=device)
    valid_np = present[episodes].cpu().numpy()
    ...
    episodes = torch.arange(n, device=device)
    bank = class_mask(classes, n, device)
    valid = present[episodes].clone()
    ...
    for begin in range(0, n, 512):
        ...
        state = causal_state(cache, episodes[sl], starts[sl], bank[sl])
        q = model(state).masked_fill(~state["mask"], -torch.inf)
        selected[sl] = q.argmax(-1)
    target = oracle_target(cache, episodes, starts, bank, valid)
